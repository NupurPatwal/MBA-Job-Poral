...
const AdminLoginPage = () => {
  ...
  const loginWithGoogle = async () => {
    try {
      const newURL = `${process.env.NEXT_PUBLIC_STRAPI_SERVER_BASE_URL}/api/admin/login/signup`;
        ...
        <div className="flex flex-col sm:flex-row sm:gap-0 gap-5 justify-between w-full min-h-screen relative">
          <div className="flex flex-col gap-5 justify-center w-full sm:w-1/3 h-full border-[1px] border-white shadow-2xl p-8 rounded-2xl">
            ...
            <div className="flex flex-col gap-3 w-full">
              <input
                value={userLoginIn.email}
                required
                onChange={inputChangeHandler}
                name="email"
                type="email"
                placeholder="Email"
                className="text-white text-[16px] border-[1px] border-white rounded-xl px-4 py-3 bg-black w-full placeholder:text-white placeholder:text-lg"
              />
              <input
                value={userLoginIn.password}
                required
                onChange={inputChangeHandler}
                name="password"
                type="password"
            ...
            <div className="flex justify-start gap-3 items-center">
              ...
              <p className="w-4 h-4 flex items-center justify-center bg-violet-700 rounded cursor-pointer peer-checked:bg-violet-900">
                ✓
              </p>
              <label htmlFor="customCheckbox" className="text-gray-400 peer-checked:text-white cursor-pointer text-[14px]">
                Remember me
              </label>
            </div>
            <div className="flex flex-col gap-2 w-full">
              <button
                onClick={loginUser}
                className="bg-[#309689] rounded-2xl text-white p-2 text-xl font-semibold text-center w-full"
              >
                Login
              </button>
              <p className="text-white text-center text-[14px]">Forgot Password ?</p>
              <Link href="/Login" className="text-white text-center text-[18px] bg-black p-2 rounded-2xl border-2 border-white">
                User Login
              </Link>
            </div>
            ...
            <div className="flex justify-center items-center gap-5">
              <Image
                ...
                height={50}
                alt="Google login"
                className="w-10 h-10 rounded-full bg-[#0F0F0F] cursor-pointer"
              />
            </div>
            <Link href="/AdminSignUp" className="text-white text-center text-[14px]">
